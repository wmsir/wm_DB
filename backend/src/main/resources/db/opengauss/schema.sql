-- ================================================================
-- WMdb 智能云平台 - 华为 openGauss / 高斯数据库 (openGauss / PostgreSQL) 全量表结构
-- 适用引擎：openGauss 2.0+ / 3.0+ / 5.0+ / 统信 UOS / PostgreSQL 12+
-- ================================================================
...
CREATE TABLE IF NOT EXISTS sys_role (
  ...
  CONSTRAINT uk_og_role UNIQUE (tenant_id, role_code)
);
...
CREATE TABLE IF NOT EXISTS sys_user (
  ...
  CONSTRAINT uk_og_user UNIQUE (tenant_id, username)
);
...
CREATE TABLE IF NOT EXISTS resource_group (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT uk_og_group UNIQUE (tenant_id, group_name)
);
...
CREATE TABLE IF NOT EXISTS db_instance (
  ...
  CONSTRAINT uk_og_inst UNIQUE (tenant_id, name)
);
